...
model_t NPP_1D = {.init = &npp_1D_init,
                  .finish = &npp_1D_destroy,
                  .fnc_spec = &npp_1D_spec,
                  .fnc_pois = &npp_1D_pois,
                  .fnc_time = NULL,
                  .jac_spec_local = &npp_1D_jac_spec_local,
                  .jac_pois_local = &npp_1D_jac_pois_local,
                  .jac_time = NULL,
                  .jac_spec_neighbour = &npp_1D_jac_spec_neighbour,
                  .jac_pois_neighbour = &npp_1D_jac_pois_neighbour,
                  .fnc_flux = &npp_1D_flux,
...
PetscErrorCode npp_1D_flux(face_t *face, app_t *ctx)
{
    volume_t *v1, *v2;
    v1 = face->nvol[0];
    v2 = face->nvol[1];
...
PetscErrorCode npp_1D_init(app_t *ctx)
{
    ...
    /* Create Jacobian */
    ierr = npp_1D_create_matrix(ctx); CHKERRQ(ierr);
...
PetscErrorCode npp_1D_create_matrix(app_t *ctx)
{
    ...
    for(i = 0; i < nr; i++) {
        ...
        o_nnz[i] = 2 + 2 * ctx->p->z[i]==0?0:1;
    }
    d_nnz[0] = 2;
    o_nnz[0] = 2 + 2 * ctx->p->z[i]==0?0:1;
    */
    unsigned int vi, j;
