#include "npp_1D_reac.h"

model_t NPP_1D_REAC = {.init = &npp_1D_reac_init,
                  .finish = &npp_1D_reac_destroy,
                  .fnc_spec = &npp_1D_reac_spec,
                  .fnc_pois = &npp_1D_reac_pois,
                  .fnc_time = NULL, /* Steady state model */
                  .jac_spec_local = &npp_1D_reac_jac_spec_local,
                  .jac_pois_local = &npp_1D_reac_jac_pois_local,
                  .jac_time = NULL, /* Steady state model */
                  .jac_spec_neighbour = &npp_1D_reac_jac_spec_neighbour,
                  .jac_pois_neighbour = &npp_1D_reac_jac_pois_neighbour,
                  .fnc_flux = &npp_1D_reac_flux,
        ...
        unsigned int i, app_t *ctx)
{
    ...
    for(f = 0; f < v->num_faces; f++) {
        ...
        } else if(nv->type == VOLTYPE_ELECTRODE) {
            c_n = c;
            phi_n = nv->state[p->num_species];
            continue;
        } else {
        ...
        app_t *ctx)
{
    ...
    for(f = 0; f < v->num_faces; f++) {
        ...
        volume_t *nv = v->nvol[f];
        PetscScalar phi_n;
        if((nv->type == VOLTYPE_BULK) || (nv->type == VOLTYPE_ELECTRODE)) {
            phi_n = nv->state[p->num_species];
        } else {
            unsigned int ni = nv->index * model->bs;
            phi_n = in[ni+p->num_species];
        ...
        PetscScalar *dst, unsigned int i, unsigned int f, app_t *ctx)
{
    ...
    dst[i] = dc;
    if(nv->type == VOLTYPE_ELECTRODE) {
        dst[i] = 0.0;
        dst[p->num_species] = 0.0;
        return 0;
    }
    dst[p->num_species] = dphi;
    return 0;
...
PetscErrorCode npp_1D_reac_flux(face_t *face, app_t *ctx)
{
    volume_t *v1, *v2;
    v1 = face->nvol[0];
    v2 = face->nvol[1];
...
PetscErrorCode npp_1D_reac_init(app_t *ctx)
{
    ...
    /* Create Jacobian */
    ierr = npp_1D_reac_create_matrix(ctx); CHKERRQ(ierr);
    ...
    /* Set update function */
    ierr = SNESSetApplicationContext(model->snes, ctx); CHKERRQ(ierr);
    ierr = SNESSetUpdate(model->snes, update_EMP); CHKERRQ(ierr);
    return 0;
}
